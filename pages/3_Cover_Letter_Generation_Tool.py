...
if(st.button("Generate", type="primary", disabled=resume is None or len(job_description) == 0, use_container_width=True, icon=":material/send:")):
    ...
    cover_letter_generator_prompt = helper.build_cover_letter_generator_promt(resume_content, job_description)
    with st.spinner("Generating Cover Letter"):
                cover_letter = helper.llm_call(cover_letter_generator_prompt)
                st.divider()
                b64_content = base64.b64encode(cover_letter.encode()).decode()
                st.markdown(f'<a href="data:file/txt;base64,{b64_content}" download="Cover Letter.txt" style="text-decoration:none;color:white;background:#007BFF;padding:10px 15px;border-radius:5px;">Download Your Cover Letter</a>', unsafe_allow_html=True)